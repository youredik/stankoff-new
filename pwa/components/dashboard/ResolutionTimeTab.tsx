...
import {
  exportResolutionTime,
  type ResolutionTimeResponse,
} from '../../services/analyticsService';

const SLA_MINUTES = 2880; // 48 hours
...
interface ResolutionTimeTabProps {
  fromDate: string;
  toDate: string;
  userId?: number;
  data: ResolutionTimeResponse | null;
  onDataLoaded: (data: ResolutionTimeResponse) => void;
  loading: boolean;
}

export const ResolutionTimeTab = ({fromDate, toDate, userId, data, onDataLoaded, loading}: ResolutionTimeTabProps) => {
  ...
  const handleExport = useCallback(async () => {
    ...
    setError(null);
    try {
      await exportResolutionTime(fromDate, toDate, userId);
    } catch (err: any) {
  ...
  return (
    <Box>
      ...
      <Box sx={{display: 'flex', alignItems: 'center', gap: 1.5, mb: 2, flexWrap: 'wrap'}}>
        ...
        <Chip label={`В нормативе (≤48ч): ${data.withinSla}`} color="success" variant="outlined" />
        <Chip label={`Просрочено (>48ч): ${data.overdue}`} color="error" variant="outlined" />
        {data.rows.length > 0 && (
          <Button
            variant="outlined"
            size="small"
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{fontWeight: 600, minWidth: 140}}>Время решения</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {data.rows.map((row) => {
                const minutes = Number(row.resolution_time_minutes);
